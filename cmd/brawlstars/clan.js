...
module.exports = {
    name: "clan",module: "brawlstars",
    aliases: ['club'],
    usage: "<tag>",
    execute: async (message,args) => {
        let option = args[0]
        if(!option) {
            return message.reply("Input the clan's token.")
        } else {
            try {
            ...
            .addField(`Members (Online ${search.onlineMembers}/${search.members.length})`,
            `Best member:\n${bestmember[0].name} - Trophies ${bestmember[0].trophies}`, true)
